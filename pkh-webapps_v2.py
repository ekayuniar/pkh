...
Aplikasi berbasi Web untuk memprediski (mengklasifikasi) penerima bantuan PKH bagi masyarakat Desa Jombok \n
""")
...
def run():
    u = {4:'LANSIA (46 - >65 TAHUN)', 3:'ANAK-ANAK (5 - 11 TAHUN)', 2:'REMAJA (12 - 25 TAHUN)', 1:'DEWASA (26 - 45 TAHUN)'}
    us = list(u.keys())
    ...
    p = {7:'PELAJAR/MAHASISWA', 6:'MENGURUS RUMAH TANGGA', 5:'BURUH LEPAS', 4:'BURUH TANI', 3:'PETANI/PEKEBUN', 2:'KARYAWAN SWASTA', 1:'WIRASWASTA', 0:'GURU/PNS/TNI/POLRI'}
    pk = list(p.keys())
    pekerjaan = st.selectbox('PEKERJAAN', pk, format_func=lambda x: p[x])
    
    ph = {4:'TIDAK BERPENGHASILAN', 3:'< 1.000.000', 2:'1.000.000 - 2.000.000', 1:'> 2.000.000'}
    phs = list(ph.keys())
    ...
    st.subheader('Probabilitas Hasil Prediksi (Klasifikasi Penerima Bantuan PKH)')
    st.write(pred_prob)
